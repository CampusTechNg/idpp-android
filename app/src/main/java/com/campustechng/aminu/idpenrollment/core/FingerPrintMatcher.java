...
/**
 * Created by Muhammad Amin on 4/3/2017.
 */
...
public class FingerPrintMatcher {
    ...
    //public static int FP_IMAGE_WIDTH = 323;
    //public static int FP_IMAGE_HEIGHT = 352;
    //Used by template
    ...
    public static int FP_MATCH_POINT_DISTANCE_MOVEMENT = 10;
    //the max rotation to use when comparint two points to count as a match
    public static int FP_MATCH_POINT_ROTATION_MOVEMENT = 10;//10;
    //a percentage
    ...
    public FingerPrintMatcher(int width , int height )
    {
    ...
    public FingerPrintMatcher(int width , int height , int MatchPointDistanceMovement , int MatchPointRotationMovment , int MatchThreshold )
    {
        FP_IMAGE_WIDTH = width;
        FP_IMAGE_HEIGHT = height;
    ...
    public void setFingerPrintImage(Bitmap m_image)
    {
        for (int i = 0; i<= FP_IMAGE_WIDTH-1;i++)
        {
            ...
            {
                int c = m_image.getPixel(i,j); // new Color(m_image.getRGB(i,j));
                if ((c  <= 127) && ((c << 16)  <= 127) && ((c << 8)  <= 127))
                {
                    P[i][j] = 1;
                }
                else
    ...
    {
        Bitmap m_ImageBuffer = Bitmap.createBitmap(FP_IMAGE_WIDTH,FP_IMAGE_HEIGHT, Bitmap.Config.ARGB_8888); // new BufferedImage(FP_IMAGE_WIDTH,FP_IMAGE_HEIGHT,BufferedImage.TYPE_INT_RGB );
        for (int i = 0; i<= FP_IMAGE_WIDTH - 1;i++)
        {
            for (int j = 0;j<= FP_IMAGE_HEIGHT - 1;j++)
            {
                if (P[i][j] == 1)
                    m_ImageBuffer.setPixel(i,j, Color.rgb(0,0,0)); //setRGB(i,j,0);
                else
                    m_ImageBuffer.setPixel(i,j,Color.WHITE); //setRGB(i,j,Color.white.getRGB()  );
            }
        }
...
*/
    ...
    public static boolean Match(double[] finger1,double[] finger2 , int threshold , boolean fastmatch)
    {
        ...
        for (int k = -1*FP_MATCH_POINT_ROTATION_MOVEMENT; k <= FP_MATCH_POINT_ROTATION_MOVEMENT; k++)
        {
            ...
            {
                //found match
                Log.i("print matcher",String.valueOf((int)((matchcount/matchcounttotal)*100)));
                return true; // (int)((matchcount/matchcounttotal)*100);
            }
            else
            {
                //not found match
        ...
        Log.i("print matcher",String.valueOf((int)((matchcount/matchcounttotal)*100)));
        return (int)((bestmatch/matchcounttotal)*100) >= threshold;
    }//end Match
